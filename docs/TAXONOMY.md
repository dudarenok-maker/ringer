...
Every primary scoreboard table uses exactly these columns, in this order: **Model | Lab | Harness | API/Plan | Tier | Tasks | First try | Pass | Tokens (median) | Tokens (total) | In | Out | Cached | Speed (median) | Last used | Notes**. The two token columns answer different questions and are not reconcilable against each other: the median is a quality statistic over scored attempts only, while the total is a spend figure over every attempt recorded for the model, including the ones excluded from scoring, so `median x tasks` will not reproduce it. In / Out / Cached break that total down and sum to it. **In is input the provider had to PROCESS — uncached input plus cache writes — not the raw `input_tokens` field.** Anthropic reports `input_tokens` as only the part no cache breakpoint covered, so a cache-heavy tool loop reports about two tokens per request: one real run recorded 276 against 196,277 of cache creation across 138 calls, and showing 276 would say the lane sent nothing. A cache write is fresh input by definition, so it belongs with In; that also makes the column comparable across engines, where the raw field is not. Cached is cache READS only — the cheap half, and usually not what a quota meters, which is why In + Out is the figure that tracks a cap while the total answers how large the run was. All four components stay stored separately regardless of how they are grouped for display. Model contains the registry display name plus the reasoning-effort suffix required below, never a raw slug or slug parenthetical. Unregistered rows derive a readable display name and keep the raw slug only in the diagnostics pointer. Notes is the most recent dated bullet read from `docs/MODEL-NOTES.md` at render time; the other dated bullets may appear in expandable detail. Attempts and failed counts are detail data, not primary columns.
